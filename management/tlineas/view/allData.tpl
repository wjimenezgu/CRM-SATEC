				...
				<table class="table table-striped table-bordered bootstrap-datatable" id="tblData">
					...
					{section name=lP loop=$LISTA_DATA} 								
						<tr style="padding:0px;" id="tr{$LISTA_DATA[lP].id}">
							...
							<td class="center">
								<a class="btn btn-primary" href="#" id="{$LISTA_DATA[lP].id}" title="Modificar Datos" data-rel="tooltip" onclick="goPage(2, this.id)"><i class="icon-edit icon-white"></i>  Modificar</a>
								<a class="btn btn-danger" href="" id="{$LISTA_DATA[lP].id}" title="Eliminar " data-rel="tooltip" onclick="goPage(3, this.id)"><i class="icon-trash icon-white"></i> Eliminar</a>
								<!--  
								<a class="btn btn-warning" href="#" id="{$LISTA_DATA[lP].id}" title="Ver " data-rel="tooltip" onclick="goPage(4, this.id)"><i class="icon-zoom-in icon-white"></i> Visualizar</a>
								 -->
								<a class="btn btn-warning" href="#" id="{$LISTA_DATA[lP].id}" title="Detalles" data-rel="tooltip" onclick="goPage(5, this.id)"><i class="icon-zoom-in icon-white"></i> Detalles</a>
							</td>
						</tr> 
					{/section}
				</table>  
