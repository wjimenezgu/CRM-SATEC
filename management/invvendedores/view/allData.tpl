		...
		<!-- 	<a class="btn btn-small btn-info" href="prnListData.php" target="__blank" title="Imprimir Datos" data-rel="tooltip" style="margin-bottom:12px;"><i class="icon-print icon-white"></i> Imprimir</a>  -->
				<table class="table table-striped table-bordered bootstrap-datatable" id="tblData">
					...
					{section name=lP loop=$LISTA_DATA} 								
						<tr style="padding:0px;" id="tr{$LISTA_DATA[lP].userVendedor}">
                            <td style="width:10%;font-family:monospace;" align="left">{$LISTA_DATA[lP].userVendedor}</td>
                            <td style="width:10%;font-family:monospace;" align="left">{$LISTA_DATA[lP].passVerificador}</td>
                            <td style="width:50%;font-family:monospace;" align="left">{$LISTA_DATA[lP].nombreVendedor}</td>
							<td style="width:30%;font-family:monospace;" class="center">
								<a class="btn btn-primary" href="#" id="{$LISTA_DATA[lP].userVendedor}" title="Modificar;" data-rel="tooltip" onclick="goPage(2, this.id)"><i class="icon-edit icon-white"></i>  Modificar</a>
						<!--    <a class="btn btn-danger" href="#" id= "{$LISTA_DATA[lP].userVendedor}" title="Eliminar;" data-rel="tooltip" onclick="goPage(3, this.id)"><i class="icon-trash icon-white"></i> Eliminar</a>   -->	
								<a class="btn btn-warning" href="#" id="{$LISTA_DATA[lP].userVendedor}" title="Ver;" data-rel="tooltip" onclick="goPage(4, this.id)"><i class="icon-zoom-in icon-white"></i> Visualizar</a>
							</td>
						</tr> 
					{/section}
				</table>  
