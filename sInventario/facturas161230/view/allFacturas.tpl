					...
					<table class="table table-striped table-bordered bootstrap-datatable" id="tblFacturas">
						...
						{section name=lP loop=$LISTA_FACTURAS}
							<tr style="padding:0px;" id="tr{$LISTA_FACTURAS[lP].idFactura}">
                                ...
                                <td class="center">
									<a href="#" id="{$LISTA_FACTURAS[lP].idFactura}|{$LISTA_FACTURAS[lP].numDocu}|{$LISTA_FACTURAS[lP].Total}|{$LISTA_FACTURAS[lP].nombre}|{$ID_TIENDA}|{$LISTA_FACTURAS[lP].fecFactura}" {IF $LISTA_FACTURAS[lP].Estado == 'ANULADA' || $LISTA_FACTURAS[lP].estadoImp == 'N'}hidden='true'{/if} Title="Anular Factura" onclick="goPage(1, this.id)"><i class="icon icon-color icon-cancel"></i></a>
        ...
        if(errors != false){ 
            ...
            if ($(".vendedorvalido").attr("id") == "0" ){
	    		...
	    		if ($('#fecFactura').val() != $('#fecHoy').val()){
	    			...
	    			// factura de otro dia
	    			window.location.href ='../notascr/NotaCr.php?action=1'+"&id="+detFactura[0]+"&numDocuFact="+detFactura[1]+"&total="+detFactura[2]+"&nombre="+detFactura[3]+"&idTienda="+detFactura[4];	    			
	    		}else{	    			
