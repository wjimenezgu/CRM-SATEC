<?php
    ...
    foreach ($detalleDocu as $dato){
        $cmds .= $dato["codExterno"] . substr($tiraEsp,1,15);
        $cmds .= $dato["cantidad"]. substr($tiraEsp,1,5);
        $cmds .= str_replace($simbolos,$newsimbolos,substr($dato["desArticulo"],0,60)).substr($tiraEsp,0,60-strlen($dato["desArticulo"])).'('.$dato["codArti"].')';
