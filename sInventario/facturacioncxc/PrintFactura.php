<?php 
    ...
    $headerDocu = array(
    					array(
    						...
    						'fecFactura' 		=> $vArray[1],
    						'idCliente' 		=> $vArray[2],
    						'nombre' 			=> urldecode($vArray[3]),
    						'telefono' 			=> $vArray[4],
    						'direccion' 		=> urldecode($vArray[5]),
    						'numOC' 			=> $vArray[6],
    						'obs' 				=> $vArray[7],
    						'indMontoLetras' 	=> $vArray[8],
    						'tipoCambio' 		=> $vArray[9],
    						'TotalN' 			=> $vArray[10],
    						'subTotalArticulos' => $vArray[11],
    						'descuento' 		=> $vArray[12],
    						'subTotal' 			=> $vArray[13],
    						'impuesto' 			=> $vArray[14],
    						'Total' 			=> $vArray[15],
    						'plazoCredi' 		=> $vArray[16],
    					)
    				);  
    ...
    $infoModoGob = array(
    					array(
    							'posPresupuestaria' => urldecode($vArray[0]),
    							'fondo' 			=> urldecode($vArray[1]),
    							'areaFuncional' 	=> urldecode($vArray[2]),
    							'solPedido' 		=> urldecode($vArray[3]),
    							'ctaCliente' 		=> urldecode($vArray[4]),
    							'tramiteCompra' 	=> urldecode($vArray[5]),
    					)
    				);
...
// echo $myString;exit;
    $vArray 	= explode('|', $myString);
    $length 	= count($vArray);
    $numArti 	= $length / 8; // entre 8 por que por cada articulo vienen 8 datos.
    $numArti 	= (int)$numArti;
    $posArray 	= 0;
    $detalleDocu = array();
    for ($i = 0; $i < $numArti; $i++) {
    	$detalleDocu[] = array(
	    						...
	    						'PorceDesc' 	=> trim($vArray[$posArray+4]),
	    						'PrecioVenta' 	=> trim($vArray[$posArray+5]),
	    						'PrecioTotalSinDesc' => trim($vArray[$posArray+6]),
	    						'PrecioVenta' 	=> trim($vArray[$posArray+7]),
	    						);
    						
    				   
    	$posArray = $posArray + 8; // el offset son los 8 elementos del articulo
    }
...
// ***************************************************************************************************
    $tempFC 		=  explode("-", $headerDocu[0]["fecFactura"]);
    $fechaA    		=  "$tempFC[2]";
    $fechaM    		=  "$tempFC[1]";
    $fechaD    		=  "$tempFC[0]";
    ...
    $cmdsD .= $newLine . $newLine;
    $cmdsD .= substr($tiraEsp,1,10) . str_replace($simbolos,$newsimbolos,$headerDocu[0]["nombre"]).substr($tiraEsp,1,10)."(".$headerDocu[0]["idCliente"].")";  //CLIENTE
    $cmdsD .= $newLine;
    
    $cmdsD .= substr($tiraEsp,1,20) . substr($tiraEsp,1,73) .
    substr($tiraEsp,1,10-strlen($dato["telefono"])) .substr($headerDocu[0]["telefono"],0,10).
    substr($tiraEsp,1,7) . $headerDocu[0]["numOC"];  // TELEFONO Y NUMOC
    // linea original
    ...
    foreach ($detalleDocu as $dato){
    	...
    	// 4 caracteres para la cantidad y 4 espacios despues
  //  	$cmdsD .= substr($tiraEsp,1,3-strlen($dato["cantidad"])).$dato["cantidad"] .substr($tiraEsp,1,5);
        $cmdsD .= substr($tiraEsp,1,4-strlen($dato["cantidad"])).$dato["cantidad"] .substr($tiraEsp,1,4);
    	...
    	if (strlen($dato["desArticulo"]) > 50){
    		$largo = 50;
    	}else{
    		$largo = strlen($dato["desArticulo"]);
    	}

    	$cmdsD .= 	str_replace($simbolos,$newsimbolos,substr($dato["desArticulo"],0,50)).
    				substr($tiraEsp,0,50-$largo).
    				substr($tiraEsp,0,2).
    	...
    	$cmdsD .= substr($tiraEsp,1,15-strlen($dato["PrecioVenta"])).$dato["PrecioVenta"].substr($tiraEsp,1,9);
    	$cmdsD .= substr($tiraEsp,1,15-strlen($dato["PrecioTotalSinDesc"])).$dato["PrecioTotalSinDesc"];
    	$cmdsD .= $newLine;
    	++$contLineas ;    	
    }
    ...
    $cmdsD .= $SaltarLineas;
    $cmdsD .= substr($tiraEsp,1,90) ."       Total:               " .substr($tiraEsp,1,14-strlen($headerDocu[0]["subTotalArticulos"])).$headerDocu[0]["subTotalArticulos"];
    $cmdsD .= $newLine ;
    $cmdsD .= substr($tiraEsp,1,90) ."       Descuento:           " .substr($tiraEsp,1,14-strlen($headerDocu[0]["descuento"])).$headerDocu[0]["descuento"];
    $cmdsD .= $newLine . $newLine ;
    ...
    substr($tiraEsp,1,14-strlen($headerDocu[0]["subTotal"])).
    ...
    $cmdsD .= $newLine . $newLine ;
    $cmdsD .= substr($tiraEsp,1,90) ."                            ".substr($tiraEsp,1,14-strlen($headerDocu[0]["impuesto"])).$headerDocu[0]["impuesto"];
    $cmdsD .= $newLine . $newLine ;
    $cmdsD .= substr($tiraEsp,1,90) ."                            ".substr($tiraEsp,1,14-strlen($headerDocu[0]["Total"])).$headerDocu[0]["Total"];
