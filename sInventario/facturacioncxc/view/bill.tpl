								...
								<div class="controls">
									    <select data-placeholder="Ingrese Cliente" style="width:100%;"  id="selectCliente"    name ="selectCliente"  data-rel="chosen"  >
                            				...
                            				{section name=lP loop=$LISTA_CLIENTES}
                                				<option {if $LISTA_CLIENTES[lP].idEstadoCliPro eq '3'} style='background-color: red;'{/if}{if $LISTA_CLIENTES[lP].idEstadoCliPro eq '2'} style='background-color: yellow;'{/if} value="{$LISTA_CLIENTES[lP].id}|{$LISTA_CLIENTES[lP].nombre}|{$LISTA_CLIENTES[lP].idEstadoCliPro}">{$LISTA_CLIENTES[lP].nombre}</option>
                                			{/section}
                            			</select>
                                </div>
                            </div>                                                                  
                    ...
                    <table class="table table-condensed">
                      <tbody> 
                        ...
                        <!--  <th>CUENTA CLIENTE</th>    --> 
                              ...
                              <td>
                                  <div class="pull-left control-group" style="margin-right:3px; width:70%;">
                                      <input type="text" class="input-block-level input-xlarge" id="fCtaClie" name="fCtaClie" maxlength="90"; value="CUENTA CLIENTE: 15100010010447066" >
                                  </div>
                              </td>
                          </tr>
...
$(document).ready(function(){
    ...
    function mathB(){
        ...
        for(var x=1; x<table.rows.length; x++){            
            ...
            vcostoUti = vcostoUti * 1.00;
            if (descuento > maxDesc){            	
            	$("#req_aut_gere").val("1");
                $("#msg_aut_gere").val("Descuento mayor al permitido por el vendedor");
            }
//    alert("Venta "+precioVenta+" CostoUti "+vcostoUti+" PorceUtiCosto "+vporceUtiCosto+" Desc Linea "+descuento);
    
            if (precioVenta < vcostoUti){                       			
    			$("#req_aut_gere").val("1");
                $("#msg_aut_gere").val("El precio de venta es menor al costo");    			
    		}

            precioVentaConDesc = precioVenta - (precioVenta * (descuento / 100.00));
// alert(precioVentaConDesc);
            if (precioVentaConDesc < vcostoUti){                                   
                $("#req_aut_gere").val("1");
                $("#msg_aut_gere").val("El precio con descuento es menor al costo");             
            }
  ...
  //Calcula el impuesto de toda la factura
	...
	function validateForm() {
		...
		if (cliente[2]=="3"){
			$("#req_aut_gere").val('1');
            $("#msg_aut_gere").val("Cliente moroso");
		}
    	...
    	// se va validar la cantidad de lineas que se pueden poner en el area de detalle
    	// en total solo son once lineas
    	
    	// se obtiene la cantidad de lineas de articulo que el usuario puso
    	var totalLineas = rowCount-1; // se resta 1 lineas por el encabezado	
    	// se validan las lineas de gobierno
    	if ($.trim($("#posPresupestaria").val()) != ''){
    	...
    	if ($.trim($("#areaFuncional").val()) != ''){
    		totalLineas = totalLineas + 1;
    	}
    	
    	if ($.trim($("#solPedido").val()) != ''){
    		totalLineas = totalLineas + 1;
    	}
    	
    	if ($.trim($("#tramiCompra").val()) != ''){
    		totalLineas = totalLineas + 1;
    	}
    	
    	if ($.trim($("#ctaCliente").val()) != ''){
    		totalLineas = totalLineas + 1;
    	}
    	
    	
    	// se valida si puso las observaciones
    	if ($.trim($("#fobs").val()) != ''){
    		totalLineas = totalLineas + 1;
    	}
    		
    	// se valida si puso que debe poner el monto en letras
    	
    	if($("#indMontoLetras").is(":checked")){
    		totalLineas = totalLineas + 1;
    	}
    	if (totalLineas > 11){
	    	// excede la cantidad de lineas permitidas en el detalla
			document.getElementById('message-fields').innerHTML = "<div class='alert alert-error center' style=''><strong><span class='icon icon-red icon-alert'/></span> Entre articulos, datos gobierno, obs y monto letras debe ser menor a 11 lineas.</strong></div>";
			$("#texto-buscar").focus();
			return false;
    	}
