<?php 
    ...
    $cmds .= substr($tiraEsp,1,15) . " ".str_replace($simbolos,$newsimbolos,$headerDocu[0]["nombre"]);  //CLIENTE
    $cmds .= $newLine;
    $cmds .= $newLine;
    $cmds .= substr($tiraEsp,1,15) . " ".str_replace($simbolos,$newsimbolos,$headerDocu[0]["direccion"]); //$headerDocu[0].["numFactura"];//DIRECCION
    $cmds .= substr($tiraEsp,1,65) . 'Factura No.' . $headerDocu[0]["numFactura"];
    $cmds .= $newLine;
