<?php 
    ...
    $cmds .= substr($tiraEsp,1,10) . " ".str_replace($simbolos,$newsimbolos,$headerDocu[0]["nombre"]);  //CLIENTE
    $cmds .= $newLine;
    $cmds .= substr($tiraEsp,1,20) . substr($tiraEsp,1,75) . $headerDocu[0]["telefono"] . substr($tiraEsp,1,18) . $headerDocu[0]["numOC"];  //CLIENTE
    $cmds .= $newLine;
	...
	$cmds .= $SaltarLineas;
    $cmds .= substr($tiraEsp,1,90) ."       Total:             " .substr('           ',1,12-strlen($headerDocu[0]["montototal"])).$headerDocu[0]["montototal"];
    $cmds .= $newLine ;    
    $cmds .= substr($tiraEsp,1,90) ."       Descuento:         " .substr('           ',1,12-strlen($headerDocu[0]["descuento"])).$headerDocu[0]["descuento"];
    $cmds .= $newLine . $newLine ;
    $cmds .= substr($tiraEsp,1,90) ."                            ".substr('           ',1,12-strlen($headerDocu[0]["subTotal"])).$headerDocu[0]["subTotal"];
    $cmds .= $newLine . substr($tiraEsp,1,13) ."30 Dias" . $newLine ;
