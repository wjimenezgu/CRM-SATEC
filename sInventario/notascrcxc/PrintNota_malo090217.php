<?php 
    ...
    $cmdsD .= substr($tiraEsp,1,60).'Nota No: '.$headerDocu[0]["numNota"];      //NUMERO NOTA
    ...
    $cmdsD .= substr($tiraEsp,1,102).$fechaD;               //FECHA FACTURA DIA
    $cmdsD .= substr($tiraEsp,1,15).$fechaM;                //FECHA FACTURA MES
    $cmdsD .= substr($tiraEsp,1,16).$fechaA;                //FECHA FACTURA ANIO 121
    //FECHA FACTURA
    
    $cmdsD .= $newLine . $newLine . $newLine . $newLine . $newLine ;
    $cmdsD .= substr($tiraEsp,1,15) . " ".str_replace($simbolos,$newsimbolos,$headerDocu[0]["nombre"]);  //CLIENTE
    $cmdsD .= $newLine;
    $cmdsD .= $newLine;
    $cmdsD .= substr($tiraEsp,1,15) . " ".str_replace($simbolos,$newsimbolos,$headerDocu[0]["direccion"]); //$headerDocu[0].["numFactura"];//DIRECCION
    $cmdsD .= substr($tiraEsp,1,65) . 'Factura No.' . $headerDocu[0]["numFactura"];
    $cmdsD .= $newLine;
    $cmdsD .= $newLine . substr($tiraEsp, 1,28) . $headerDocu[0]["nombreVendedor"];
    $cmdsD .= substr($tiraEsp, 1,40) . $headerDocu[0]["numOC"];
    
    
    $cmdsD .= $newLine . $newLine . $newLine . $newLine ;
     
    foreach ($detalleDocu as $dato){
    	$cmdsD .= substr($tiraEsp,1,3) . $dato["cantidad"] . substr($tiraEsp,1,15);
    	$cmdsD .= $dato["codArti"]. substr($tiraEsp,1,8);
    	$cmdsD .= str_replace($simbolos,$newsimbolos,substr($dato["desArticulo"],0,70)).substr($tiraEsp,0,70-strlen($dato["desArticulo"]));
    	$cmdsD .= substr($tiraEsp,1,2);
    	$cmdsD .= substr('           ',1,12-strlen($dato["PrecioVenta"])).$dato["PrecioVenta"].substr($tiraEsp,1,10);
    ...
    $cmdsD .= $newLine ;
    $cmdsD .= substr($tiraEsp,1,32).$headerDocu[0]["razonNota"].$newLine;
    ++$contLineas;
    ...
    $cmdsD .= $newLine ;
    $cmdsD .= $newLine ;
    $cmdsD .= substr($tiraEsp,1,123) .substr('           ',1,12-strlen($headerDocu[0]["subTotal"])).$headerDocu[0]["SubTotal"];
    $cmdsD .= $newLine . $newLine . substr($tiraEsp,1,130) .substr('           ',1,12-strlen($headerDocu[0]["impuesto"])).$headerDocu[0]["impuesto"];
    $cmdsD .= $newLine . $newLine . substr($tiraEsp,1,130) .substr('           ',1,12-strlen($headerDocu[0]["Total"])).$headerDocu[0]["Total"];
