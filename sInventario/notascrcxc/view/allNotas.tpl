				...
				<div class="box-content">
					<table class="table table-striped table-bordered bootstrap-datatable" id="tblFacturas">
						...
						{section name=lP loop=$LISTA_NOTAS}
							<tr style="padding:0px;" id="tr{$LISTA_NOTAS[lP].idNota}">
                                <td style="width:385px; text-align:left"> 
                                	<a style="color:#004C66;" href="#" 
                                		id="{$LISTA_NOTAS[lP].idNota}|{$LISTA_NOTAS[lP].nombre}|{$LISTA_NOTAS[lP].userVendedor}|{$LISTA_NOTAS[lP].subTotal}|{$LISTA_NOTAS[lP].descuento}|{$LISTA_NOTAS[lP].impuesto}|{$LISTA_NOTAS[lP].Total}|{$LISTA_NOTAS[lP].razonNota}|{$LISTA_NOTAS[lP].Estado}|{$LISTA_NOTAS[lP].estadoImp}|{$LISTA_NOTAS[lP].NumNota}|{$LISTA_NOTAS[lP].numDocuFact}"
                                		onclick="getDet(this.id)">{$LISTA_NOTAS[lP].nombre}
                                	</a>
                                </td>
     							<td style="width:40px;  text-align:right">{$LISTA_NOTAS[lP].idNota}</td>
     							<td style="width:70px;  text-align:right">{$LISTA_NOTAS[lP].NumNota}</td>
     							<td style="width:70px;  text-align:right">{$LISTA_NOTAS[lP].numDocuFact}</td>
     							<td style="width:150px;">{$LISTA_NOTAS[lP].fecNota}</td>
     							<td style="width:285px; ">{$LISTA_NOTAS[lP].razonNota}</td>
                                <td style="width:185px; text-align:right">{$LISTA_NOTAS[lP].Total}</td>
                                <td>{$LISTA_NOTAS[lP].estadoImp}</td>
                                <td>{$LISTA_NOTAS[lP].Estado}</td>
                                <td class="center">
							<!-- 		<a href="#" id="{$LISTA_NOTAS[lP].idNota}|{$LISTA_NOTAS[lP].userVendedor}|{$ID_TIENDA}|{$LISTA_NOTAS[lP].numDocuFact}" {IF $LISTA_NOTAS[lP].Estado == 'ANULADA' || $LISTA_NOTAS[lP].estadoImp == 'S'}hidden='true'{/if} Title="Imprimir Nota" onclick="goPage(1, this.id)"><i class="icon icon-color icon-print"></i></a> -->
							 	<a href="#"  id="{$LISTA_NOTAS[lP].idNota}" {IF $LISTA_NOTAS[lP].Estado == 'ANULADA' || $LISTA_NOTAS[lP].estadoImp == 'S'}hidden='true'{/if} Title="Imprimir Nota" onclick="impNota(this.id);"><i class="icon icon-color icon-print"></i></a>  		
								</td>
