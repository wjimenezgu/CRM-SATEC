...
// echo $sql;exit;
    ...
    function savePedidoNac($action,$idPedido,$idProveedor,
    					   ...
    					   $item,$codArti,$cant,$cost,$precioNuevo,$utilNueva) {
...
// echo $sql;exit;
    	...
    	if ($message == "OK"){   	
		    	...
		    	foreach($item as $key => $value){
		    		...
		    		$vitem    = $item[$key];
                    $vutilNueva = $utilNueva[$key];
                    $vutilNueva = str_replace(',', '', $vutilNueva);
		    		$i=$i+1;
		 
		    		if ($action == '5'){
		    			//se estan recibiendo los articulos, el procesamiento es especial, por eso se usa un sp especifico
		    			$vprecio  = $precioNuevo[$key];
		    			$sql = "call spInsDetPedidoNacReci($idPedido,'$numFac',$i,'$vcodArti','$vitem',$vcant,$vcost,$vprecio,
                            $vutilNueva,$tCambio,@out_status);";
		//   echo $sql;exit;		    			 			
		    		}else{
 ...
 //   echo $sql;exit;
    ...
    function queryItemsOrdersBodegaNac($criterio, $texto) {
         
        $sql = "call spQueryItemsOrdersBodegaNac($criterio,'$texto',@out_status);";
