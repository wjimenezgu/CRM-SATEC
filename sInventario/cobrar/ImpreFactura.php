...
<input type="hidden" name="montotal"  id="montototal"  value="<?php echo $headerDocu[0]["montototal"]; ?>" />
    <input type="hidden" name="subtotal"  id="subtotal"  value="<?php echo $headerDocu[0]["subTotal"]; ?>" />
    <input type="hidden" name="descuento"  id="descuento"  value="<?php echo $headerDocu[0]["descuento"]; ?>" />
    <input type="hidden" name="impuesto"  id="impuesto"  value="<?php echo $headerDocu[0]["impuesto"]; ?>" />
    <input type="hidden" name="Total"  id="Total"  value="<?php echo $headerDocu[0]["Total"]; ?>" />

    <div style="width:100%">
         
    	<input type="button" id="imprime" autofocus name="imprime"   style="margin-left:25%; margin-top:25%;font-size:48px" onclick="javascript:jsWebClientPrint.print('cmds=' + $('#cmds').val()+'&idTienda=' + $('#idTienda').val()+'&montototal='+$('#montototal').val()+'&descuento='+$('#descuento').val()+'&impuesto='+$('#impuesto').val()+'&Total='+$('#Total').val()+'&subtotal='+$('#subtotal').val());<?php if($idTienda != 0){?>  window.location.href ='../facturacion/index.php'; <?php }if ($idTienda == 0){?>window.location.href ='../cobrar/index.php' <?php } ?>" value="Presionar Enter para regresar ..." />
